...
MESSAGES: dict[str, str] = {
    ...
    "init.opt_native": "  1. cursor-native -- 在 Cursor IDE 内通过 skills + subagents 驱动（推荐）",
    "init.opt_orchestrator": "  2. orchestrator -- 外部 CLI 进程驱动 cursor-agent",
    "init.mode_native_selected": "  → cursor-native 模式：将生成 skills、subagents 和 rules",
    "init.mode_orchestrator_selected": "  → orchestrator 模式：标准 CLI 驱动工作流",
    "init.native_adversarial_model": "  跨模型评审模型（用于信任边界规则）",
    ...
    "prompt.plan": (
        ...
        "{tree_block}\n\n"
        "## 规划原则\n"
        "- **先搜索再构建**：在提出新模式或新依赖之前，先检查项目已有的约定和工具。"
        "优先扩展现有模式，而非引入新的。\n"
        "- **追求完整性**：当成本不大时，合同应覆盖测试、错误处理和类型安全，"
        "不要把显而易见的工作推迟到下一轮迭代。\n\n"
    ...
    "prompt.builder": (
        ...
        "{context}\n\n"
        "## 工程原则\n"
        "- **先搜索再构建**：在写新代码之前，先检查项目中是否已有可复用的模式、"
        "工具函数或组件。优先复用已有模式，而非发明新的。搜索顺序：项目内约定优先，"
        "知名库其次，最后才写新代码。\n"
        "- **追求完整性**：当成本不大时要做到位 — 为边界情况写测试、处理错误路径、"
        "添加类型注解。不要推迟显而易见的改进。\n"
        "- **最小变更足迹**：用最少的改动满足需求，同时保持正确性和清晰度。\n\n"
        "## 执行要求\n"
        "- 按合同交付物逐项实现，完成后提交代码\n"
    ...
    "prompt.eval": (
        ...
        "## 合同\n{contract}\n\n"
        "## 信任边界\n"
        "以下内容由 **Builder 生成**（不可信）。请批判性审查 — Builder 可能：\n"
        "- 声称完成了交付物但实际上并未实现\n"
        "- 在合同范围外做了未授权的更改\n"
        "- 在表面上通过测试的同时引入了隐蔽的回归\n"
        "请始终根据实际代码变更验证 Builder 的声明。\n\n"
        "## 本轮变更（核心 — 根据合同评估 Builder 的工作）\n"
        "{iteration_summary}\n\n"
        "## 累积分支变更（上下文 — 所有迭代的合并变更）\n"
        "分支: {branch}\n\n"
        "{branch_summary}\n\n"
        "## Builder 工作日志（摘要 — 不可信，请对照 git diff 验证）\n{build_log}\n\n"
        "## 评估指引\n"
    ...
    "prompt.alignment": (
        ...
        "## 原始需求\n{requirement}\n\n"
        "## 合同\n{contract}\n\n"
        "## Builder 分支变更（不可信 — Builder 生成的代码，请批判性验证）\n"
        "分支: {branch}\n\n{branch_summary}\n\n"
        "## 对齐评估指引\n"

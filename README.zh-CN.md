...
|  | 导航系统 | 交通规则 | 行车记录仪 |
|--|:------:|:------:|:--------:|
|  | vision → plan → roadmap | 自适应多角色评审 + 门禁 + 信任边界 | 审计轨迹 + 学习记忆 + 回顾 |
...
就这样 — 计划、构建、自适应多角色评审、PR，一句话搞定。

**你会看到：** agent 生成 spec + 合约，多角色评审者并行挑战计划，然后 agent 实现、跑 CI、再经过自适应代码评审、开 PR — 全程自主。
...
每个任务都从 **spec + 合约** 开始 — 包含交付物、验收标准和风险分析 — 经自适应多角色审查后才编写代码。
...
| 技能 | 何时用 | 功能 |
|------|--------|------|
| `/harness-plan` | "我有个需求" | 细化计划 + 自适应审查 → 自动构建/评审/发布/回顾 |
...
| `/harness-eval` | 自适应多角色代码评审（基于 escalation score 自动选择 FAST/LITE/FULL） |
...
| `harness git-preflight [--json]` | 预检（工作树、分支） |

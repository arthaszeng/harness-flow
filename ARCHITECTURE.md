...
**`.cursor/worktrees.json`** — Cursor-native post-create worktree hook. Generated by `generate_native_artifacts()` (idempotent: skipped if file exists unless `force`). Creates symlinks (`ln -sfn` / `mklink /D`) from linked worktrees back to the main tree's shared artifacts.
...
- `worktrees.json` — Cursor-native post-create hook; creates symlinks (`ln -sfn` / `mklink /D`) from linked worktrees back to the main tree's `.harness-flow/` and `.cursor/` artifacts. Manually maintained.
